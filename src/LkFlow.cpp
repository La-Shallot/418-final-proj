...
std::vector<std::pair<float, float>> Lk_Flow::CalcLkFlow(std::vector<std::vector<float>>& imgA, std::vector<std::vector<float>>& imgB, int xsarea, int ysarea, int xarea, int yarea, std::vector<std::pair<float, float>> corners, bool verbose)
    {
        // start timer
        ...
        for (int i = 0; i < corners.size(); i++)
        {
            ...
            for (int x = srx.first; x <= srx.second; x++)
                for (int y = sry.first; y <= sry.second; y++)
                {
                    ...
                    double curIx = ((int)imgA[y][px] - (int)imgA[y][nx]) / 2;
                    double curIy = ((int)imgA[px][y] - (int)imgA[ny][x]) / 2;
                    ...
                    double curdI = ((int)imgA[y][x] - (int)imgB[y][x]);
            ...
            #pragma omp critical
            out.push_back(std::make_pair(V[0], V[1]));
            // if (verbose)
                // printf("\nFor the corner (%d,%d) - v = (%f,%f)", cur_corner.x, cur_corner.y, V[0], V[1]);
